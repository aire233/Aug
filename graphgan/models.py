...
class Generator(nn.Module):
    ...
    def forward(self, z):
        batch_size = z.size(0)
        # 生成节点属性向量，形状为 [B, max_nodes, node_dim]
        nodes = self.node_generator(z).view(batch_size, self.max_nodes, self.node_dim)
        # 向量化计算所有节点对的边概率
        ...
        edge_probs = self.edge_generator(node_pairs.view(-1, 2 * self.node_dim)).view(batch_size, -1)
        # 构造对称的邻接矩阵
