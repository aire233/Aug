...
# 加载数据
edges = load_adjacency_matrix(f"{folder_path}/{ds_prefix}_A.txt")
graph_indicator = load_graph_indicator(f"{folder_path}/{ds_prefix}_graph_indicator.txt")
node_labels = load_node_labels(f"{folder_path}/{ds_prefix}_node_labels.txt")
graph_labels = load_graph_labels(f"{folder_path}/{ds_prefix}_graph_labels.txt")
...
real_loader = DataLoader(
    pyg_data_list, batch_size=BATCH_SIZE, shuffle=True, pin_memory=True
)
    ...
    for epoch in tqdm(range(start_epoch, EPOCHS), desc="Training epochs"):
        ...
        for real_data in tqdm(real_loader, desc="Batches", leave=False):
            ...
            fake_data = batch_dense_to_pyg(
                gen_nodes.detach(), gen_adj.detach(), threshold=0.5
            )
