...
def convert_to_pyg(graphs, graph_labels, max_nodes=50):
    ...
    for graph_id, graph in tqdm(graphs.items()):
        # 排序节点保证顺序一致
...
# 初始化模型
generator = Generator(max_nodes=max_nodes).to(device)
...
# 数据加载器
loader = DataLoader(pyg_data_list, batch_size=BATCH_SIZE, shuffle=True)
...
for epoch in tqdm(range(EPOCHS)):
    for real_data in loader:
        ...
        real_loss = F.binary_cross_entropy(
            discriminator(real_data), torch.ones(batch_size, 1).to(device)
        )
        ...
        for i in range(batch_size):
            ...
            data = Data(x=gen_nodes[i], edge_index=edge_index)
            fake_data_list.append(data)
        fake_data = next(iter(DataLoader(fake_data_list, batch_size=batch_size)))
        fake_data = fake_data.to(device)

        fake_loss = F.binary_cross_entropy(
            discriminator(fake_data), torch.zeros(batch_size, 1).to(device)
        )
        ...
        g_loss = F.binary_cross_entropy(
            discriminator(fake_data), torch.ones(batch_size, 1).to(device)
        )
